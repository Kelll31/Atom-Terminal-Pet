...
class RuleEngine:
    ...
    def load_rules(self):
        full_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), self.config_path)
    ...
    def evaluate_condition(self, condition, metrics) -> bool:
        ...
        if operator == ">": return current_value > target_value
        elif operator == "<": return current_value < target_value
        elif operator == "==": return current_value == target_value
        elif operator == ">=": return current_value >= target_value
        elif operator == "<=": return current_value <= target_value
        return False
    ...
    async def engine_loop(self):
        ...
        while self.is_running:
            try:
                ...
                metrics = {
                    "cpu": int(pc_monitor.get_gpu_usage()), # using as placeholder if we were caching, but let's just collect
                ...
                for rule in self.rules:
                    ...
                    if self.evaluate_condition(cond, full_metrics):
                        ...
                        if time_active >= duration_needed and not self.rule_state.get(trigger_key):
